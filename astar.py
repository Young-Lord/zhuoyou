# code from https://www.cnblogs.com/jin521/p/9687533.html
...
def readfile(url,x,y):
    global maze,map_border
    map_border=(x,y)
    maze=[[int(j) for j in list(i)] for i in url]
    #print(maze)
    #print(map_border)
...
def mark_path(node):
    ...
    if node.father.x-node.x>0:
        orientation.append('左')
    elif node.father.x-node.x<0:
        orientation.append('右')
    elif node.father.y-node.y>0:
        orientation.append('上')
    elif node.father.y-node.y<0:
        orientation.append('下')
...
def astar(game_map,start_y,start_x,end_y,end_x):
    #读取参数
    chang = len(game_map[0])
    kuan = len(game_map)
    readfile(game_map,chang,kuan)
    global start,end
    global orientation,open_list,close_list
